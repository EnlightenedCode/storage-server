package com.risevision.storage;

public final class EntityKind {
	
	public static final String COMPANY = "Company";

	public static final String MEDIA_LIBRARY_USAGE = "MediaLibraryUsage";
	public static final String MEDIA_LIBRARY_FILE_LOG = "MediaLibraryFileLog";
	
	// internal
	
//	public static final String ENTITY_CURSOR = "EntityCursor";
//	public static final String ENTITY_DELETED = "EntityDeleted";
//	public static final String REPORT_CSV = "ReportCSV";
	
	// Directory
	
	public static final String AUTHORIZATION_TOKEN = "AuthorizationToken";

}
